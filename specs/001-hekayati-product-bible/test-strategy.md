...
Scripted mock-provider faults per taxonomy row × retry-policy assertion; ENOSPC/EACCES/EROFS through injected real-FS/asset hooks plus a bounded dedicated-volume fixture where available; SQLite busy/unavailable; network loss; and app-kill matrix: {claimed, provider-running, prepared temp, renamed-before-DB, retry delay, waiting review, story gen, image fan-out, PDF render, import commit} × restart → SC-002 assertions (no duplicates, completed intact, fences/pauses preserved).
