...
4. **Given** a provider returns malformed structured output, **When** validation fails, **Then** the job fails with normalized category `malformed_output`, the raw payload is retained for diagnosis (with redaction rules applied), and retry policy per `contracts/job-scheduler-contract.md` governs what happens next.
...
- Character deleted while referenced by scenes → blocked with affected-scene resolution flow (FR-039).
...
- **FR-001**: System MUST support customer CRUD with: name, WhatsApp number, notes, photo-consent status (granted / not granted, with date and free-text note), created/modified timestamps.
...
- **FR-004**: System MUST block any AI generation call that would transmit photos of a customer whose consent is not recorded, stating the reason.
...
- **FR-017**: Character relationship (to the main child) and narrative role (in a specific story) MUST be separate attributes; narrative role is per-project (main hero, co-hero, companion, guide, helper, person to rescue, beginning/ending-only, custom).
...
- **FR-021**: System MUST apply EXIF orientation, then strip location (GPS) and other non-essential EXIF metadata from stored working copies. Originals are retained locally, never sent to providers.
- **FR-022**: System MUST enforce a configurable per-file size limit (default 25 MB) and validate file type by content, not extension.
- **FR-023**: System MUST warn (non-blocking) on: blurry image, face too small, multiple faces, extreme shadows, obstructed face (sunglasses etc.), heavy filter suspicion, inconsistent apparent age across references, conflicting hair/clothing across references. Warnings are advisory; the recommended-input checklist (clear front face, three-quarter, full body, clothing reference, good lighting) MUST be shown in the intake UI.
- **FR-024**: Multi-face photos MUST require the employee to mark the intended person (crop/selection) before the photo is usable as a face reference.
...
- **FR-039**: Removing/renaming/archiving a character referenced by mentions MUST surface every affected scene and require explicit resolution; renames MUST NOT break references (IDs stable).
