...
## I — Flow mode (external manual images)

| ID     | Case                                                        | Defined behavior                                                                                                                        | Refs                       |
| ------ | ----------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| EC-I01 | Import pinned to a superseded prompt/pack version           | Commit precondition rejects with `stale_dependency` + exact reason; zero state change                                                     | FR-152/155, FR-065         |
| EC-I02 | Imported image below effective print DPI at trim size       | Persistent per-page warning at import; preview allowed; FR-123 preflight remains the hard print gate; never auto-upscaled                 | FR-156, FR-123             |
| EC-I03 | Aspect ratio outside layout tolerance                       | Import warning stating crop/fit consequence; operator decides; never auto-cropped                                                         | FR-156                     |
| EC-I04 | Corrupt file / wrong extension / non-image bytes            | Magic-byte sniffing + decode validation fail → `media_decode_failure`/`invalid_input`; no partial commit                                  | FR-154, FR-092/093         |
| EC-I05 | Import targets a locked or approved page                    | Rejected with lock reason; content frozen until unlock                                                                                    | FR-155, FR-064             |
| EC-I06 | Partial pack imported (e.g., 9 of 16)                       | Mapped pages advance; the rest stay `waiting_external_import` with visible blocking reason; restart-safe                                  | FR-153/155                 |
| EC-I07 | Same page imported twice                                    | Second import appends a new illustration version; nothing overwritten; history intact                                                     | FR-155                     |
| EC-I08 | Wrong image mapped to wrong page                            | Mapping UI shows prompt + thumbnail side-by-side; mandatory FR-118 review checklist is the backstop; fix = re-import correct mapping      | FR-155, FR-118             |
| EC-I09 | Upstream edit after successful import                       | Normal invalidation matrix rows apply (imported versions are ordinary page illustration versions)                                         | IM-06/07/08, FR-158        |
| EC-I10 | Reference bundle export for a character without recorded consent | Blocked before any file is written, same reason surface as FR-004; description block remains usable | FR-151, FR-004 |
| EC-I11 | Provider switched away while imports pending                | Waiting jobs re-dispatch to the new provider only after explicit confirmation listing affected pages; imported versions preserved          | FR-159, FR-095             |
| EC-I12 | EXIF/GPS/XMP metadata in downloaded file                    | Stripped into privacy-clean working copy before the asset becomes usable; original download untouched on operator's disk                   | FR-154                     |
| EC-I13 | Flow rendered story text inside the image pixels            | FR-071 rule unchanged; review checklist blocks; operator regenerates externally with the pack's no-text instruction                        | FR-071, FR-118             |
| EC-I14 | Invisible SynthID-style watermark in Flow output            | Acceptable; not a preflight defect; visible watermarks/logos are an FR-118 review rejection                                                | FR-158, FR-123             |
| EC-I15 | Pack export requested before prompts validate               | Blocked with the pipeline-stage reason (FR-114 chain); no partial pack                                                                     | FR-150, FR-114             |
| EC-I16 | Operator tries to locate/export exact original bytes for Flow | `originals/` assets are never exportable through this feature (never provider-eligible anywhere); bundle contains only privacy-clean working copies + sheet renders; each export logged with upload warning shown | FR-151, FR-025 |
