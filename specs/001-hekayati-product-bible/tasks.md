...
- [ ] T-P6-01 Generation pipeline jobs: plan → story → scenes → prompts → page fan-out, honoring the 004-pinned operator choice for one/two sequential images without changing it, wired to dependency chain + waiting_review gates (`src/domain/story/pipeline.ts`) [FR-060/114, US4-AS1]
- [ ] T-P6-02 Append validated generated output to the 004 project/story/scene/page lineage, completing prompt/illustration/provenance/review fields without replacing manual versions; emit ChangeEvents (`src/domain/versioning/`) [FR-059, data-model hooks]
...
- [ ] T-P6-08 Consume the hash-confirmed 004 page-count structure revision for generated projects: apply IM-09 staleness, preserve locked/approved content, and schedule only the explicitly accepted add/merge/remove scope; do not recreate or bypass the 004 preflight [FR-058, IM-09]
