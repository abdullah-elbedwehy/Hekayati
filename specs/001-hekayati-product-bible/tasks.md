...
- [ ] T-P3-01 [P] [US10] Versioned template identity/version schemas + expected-head operations (create/edit/duplicate/archive/restore/disable/enable), immutable project pin reads, exact active/archived/disabled picker semantics, and IM-16/no-source-mutation tests (`src/domain/templates/`) [FR-050–052, IM-16, EC-B11]
- [ ] T-P3-02 [P] [US10] Author and validate the 7 stable-key Arabic seed templates, then install them idempotently through the Phase-1 hook without overwriting an edited, archived, or disabled seed (`src/domain/templates/seeds/`) [FR-050/053, US10-AS1]
- [ ] T-P3-03 [US3] Immutable ProjectVersion story configuration domain + Arabic RTL UI: all FR-045 fields, deterministic/versioned narration-dialogue balance suggestion with operator override, same-family pinned participants, missing/archived family-anchor rejection for new projects without changing existing pins, expected-head conflicts, and FR-014(a) project-only appearance overrides with atomic project-head + override-head + IM-04 outbox commit (`src/domain/projects/`) [FR-014/017–018/045/046/049, C-21, IM-04, CHK006/011]
- [ ] T-P3-04 [US3] Mention engine (tests first): ordered strict text/mention/group/unresolved segments, ID-bound tokens rendered from current names, NFC+tashkeel-insensitive search, spaces, duplicate disambiguation data, deterministic C-24 groups, paste/partial-delete degradation, and no stored authoritative display names (`src/domain/mentions/`) [FR-035–040, C-11/C-24, EC-A05/A06]
